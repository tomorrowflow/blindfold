...
**Addendum (issue #374, ADR-0057 D6.2): `redacted_thinking.data` is ciphertext, not prose, on
both sides.** The #323 amendment above named `redacted_thinking` in its covered-set prose as a
block type the walk now reaches, but never carved its `data` field out — so a client resending
history that echoes a `redacted_thinking` block (Claude Desktop and Claude Code both do) fed the
provider's own encrypted reasoning blob through deterministic blinding and L3 candidate
production, and the leak gate's exhaustive walk checked it too. `data` is handed back by the
provider byte-exact at decrypt time and never regenerated by anything Blindfold does; rewriting
even one byte corrupts the request irrecoverably, and a coincidental substring match inside it is
a declared collision, not a leak, by the same argument `signature` already carries. `data` is
excluded on the `redacted_thinking` block type only (`_BLOCK_TYPE_NON_HOP_KEYS`, engine.py, beside
