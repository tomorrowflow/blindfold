...
/// The menu bar app's root scene (ADR-0039/0040/0041, issue #213): icon + header (issue
/// #210) plus supervision -- Start/Stop Proxy, Quit, and the Refused remedy. Review/blocks
/// deep-links and the Unprotected-mode submenu stay separate slices (narrow on purpose,
/// per the issue). Not `@main` itself: `main.swift` decides between this and the headless
...
struct BlindfoldMenuBarApp: App {
    ...
    var body: some Scene {
        MenuBarExtra {
            MenuBarSupervisionContent(model: model)
        } label: {
...
/// own -- every label/visibility decision is a `BlindfoldCore` call.
private struct MenuBarSupervisionContent: View {
