...
def test_colliding_pool_entry_is_skipped_not_reused_and_non_colliding_entries_stay_e_stable():
    ...
    # namespace fallback (ADR-0052, issue #335 -- "Person Surrogate {N}" was the
    # old natural-language shape), at the first position past the (issue #338
    # enlarged, 32-entry) pool.
    assert minted[4:] == baseline[5:] + [f"BFP{pool_size:04d}"]
