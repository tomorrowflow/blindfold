...
- **Candidate span** — a flagged span handed to L3, plus minimal context. Two
  ...
  namespace is ever a real referent (ADR-0055). The two producers are not
...
- **Positional case heuristic** — a **Suppression** condition (ADR-0033) that
  ...
  at sentence/quotation/heading/list-marker/table-cell start (table cells:
  issue #360) in the same **hop** text, never mid-sentence in capitalized form
  (positional evidence) — *and either* (a) it appears lowercase elsewhere in
  the same hop (vocabulary evidence) *or* (issue #161) at least one occurrence
  sits at a list/numbered-marker start specifically, not a bare heading,
  unmarked paragraph start, or table cell (list-marker evidence, for a one-off
  bullet/skill-list command name that never recurs lowercase — table-cell
  position deliberately never substitutes for vocabulary evidence, because
  tables are where genuine names concentrate). The positional gate is always
  load-bearing: it guards the **Don/Mark/Stone failure mode** — a real first
  name appearing mid-sentence in capitalized form always fails (b) and is
  never suppressed, regardless of which of (a)/list-marker evidence would
  otherwise fire. English-benefiting, German-neutral: German capitalizes all
  nouns mid-sentence, so vocabulary evidence rarely fires for German
  vocabulary and German candidates pass through unchanged.
