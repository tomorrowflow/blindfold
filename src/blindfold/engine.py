...
# Non-hop keys that only apply to one specific block ``type`` (issue #374, ADR-0057
# D6.2), layered on top of the flat _BLOCK_NON_HOP_KEYS above. "data" cannot join
# that flat set: it is ordinary prose on other block shapes (a `document`'s
# embedded source), so excluding it everywhere would hide real entities elsewhere.
# On a `redacted_thinking` block specifically, `data` is the provider's own
# encrypted reasoning blob -- handed back byte-exact at decrypt time and never
# regenerated by anything Blindfold does, never prose -- the same class of
# argument `signature` carries above, just type-scoped rather than global.
# ADR-0051's #323 amendment named `redacted_thinking` in its covered-set prose
# without carving out this field; this closes that gap. Closed and enumerated the
# same way: a new entry needs the same kind of argument made here.
_BLOCK_TYPE_NON_HOP_KEYS: dict[str, frozenset[str]] = {
...
def _strip_block_type_non_hop_fields(node: Any, forbidden: list[str]) -> Any:
    ...
    Mirrors :func:`_strip_schema_structural_tokens`'s recursive walk and is symmetric
    with the blinder's own generic recursive walk (:func:`_blindfold_block_value`,
    ...
    if isinstance(node, dict):
        ...
        # `type` is a block-type discriminator only when it's a string -- inside a
...
def _gate_excluded_view(payload: dict[str, Any]) -> tuple[dict[str, Any], str]:
    ...
    Issue #374 (ADR-0057 D6.2): a block-type-specific non-hop field (today, only
    ``redacted_thinking.data`` -- provider-encrypted ciphertext, never rewritten by
    the blinder) is excluded the same way: a coincidental real-value substring match
    ...
    # property can legally be named "type" and hold a nested schema dict (not a
    # block-type string), which _strip_block_type_non_hop_fields's `.get("type")`
