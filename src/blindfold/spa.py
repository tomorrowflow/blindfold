"""Single-file management SPAs for the entity list (ADR-0011).

Small Vue 3 pages rendered straight out of FastAPI, consuming
:mod:`blindfold.app` management endpoints directly. The review-inbox page
that used to live here (issue #14) is retired — it's migrated into the
unified shell at ``/ui/inbox`` (issue #99). The org-graph page is also
retired — migrated into the unified shell at ``/ui/graph`` (issue #98).
The JSON endpoints both consumed are unchanged and now serve the shell's
React views instead.

Embedded as a Python string (rather than mounted via :class:`StaticFiles`) so
the proxy stays a one-process install and each page is testable with the
...
# NOTE: org_graph_html() / _ORG_GRAPH_HTML removed by issue #98 — the legacy
# CDN-loaded Cytoscape page at /ui/org-graph is retired and replaced by the
# shell's vendored React GraphEditor view at /ui/graph (ADR-0026).
