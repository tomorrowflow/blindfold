...
``run(argv, *, store)``); ``main()`` wires those from the real environment --
``BLINDFOLD_EXCHANGE_CAPTURE_DIR``, the shared-store refusal every devtools
entry point carries (ADR-0047 §7), and the operator's own real entity graph
(read-only) and L3 detector, never the vendored seed the shipped proxy
blindfolds with by default -- validating replay against Sample data would
